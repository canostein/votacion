...
function cargar_pantalla_inicial(){
     ...
     * Toma la decision de quÃ© pantalla mostrar segun el contexto. Las salidas
    ...
    // Si tenemos configurados quÃ© botones usamos.
    if(botones !== null){
        ...
        if(!cantidad){
            // si los botones estan establecidos pero el array estÃ¡ vacio
            // vamos a votar por categorias.
            fallback = true;
        } else if (cantidad == 1){
            // Si tenemos un solo boton configurado seleccionamos ese modo, por
            // que para quÃ© mostrar una pantalla con un solo boton.
...
function _cargar_pantalla_lista_completa(cod_partido){
    ...
    if(typeof(cod_partido) !== "undefined"){
        // Si el codigo del partido estÃ¡ definido Buscamos todas las listas
...
function _cambiar_categoria(cod_categoria){
    /* Cambia de categoria que estÃ¡ siendo elegida.
     * Argumentos:
...
function _cargar_candidatos_categoria(categoria, agrupacion){
    ...
    if(!agrupacion && muchos_candidatos){
        ...
        // En caso de que estÃ© mostrando un agrupacion y/o los candidatos sean
...
// El diccionario con todo lo que seleccionÃ³ el elector.
var _seleccion = {};
...
function toggle_array_candidato(max, boton, array, codigo) {
     ...
     */
    if (constants.cambiar_rapido_seleccion && max == 1 && array.length == 1 && !array.includes(codigo)) {
        let seleccionados = $(".opcion-preferencia.seleccionado");
...
function tiene_un_unico_preferente(candidato) {
    return !candidato.cargo_ejecutivo && candidato.secundarios.length === 0;
}
...
function seleccionar_candidatos(categoria, codigos){
    ...
    // Esto significa que hay preferencias para esa categoria y podrÃ­a ser
    // util ingresar automÃ¡ticamente
    ...
    if ((typeof(categoria.max_preferencias) !== "undefined")
        ...
        if (preferencias_tmp == null) {
            ...
            if (tiene_un_unico_preferente(candidato)) {
                console.log("SALTEARRRRRRRR", _seleccion.preferencias);
                ...
                console.log("SALTEARRRRRRRR despues", _seleccion.preferencias);
            } else {
    ...
    // Esto significa que hay tachas para esa categoria y podrÃ­a ser
    // util ingresar automÃ¡ticamente
...
function seleccionar_lista(cod_lista){
    ...
    if(typeof(boleta) === "undefined"){
        ...
        // si la boleta estÃ¡ definida quiere decir que  tenemos que seleccionar
...
function _crear_candidato_secundario(candidato_principal, id_umv){
    var candidato_secundario = null;
    for (var i in candidato_principal.secundarios_datos_extra) {
        var candidato = candidato_principal.secundarios_datos_extra[i];
        if (candidato.id_candidatura == id_umv) {
            candidato_secundario = candidato;
        }
    }

    if (!candidato_secundario) return
    candidato_secundario.lista = candidato_principal.lista;
    candidato_secundario.asistida = candidato_secundario.nombre;
    return candidato_secundario
}
...
function _cargar_confirmacion(){
    ...
    for(var i in categorias){
        ...
        for(var j in candidatos){
            ...
            if (typeof(categoria.max_preferencias) !== "undefined") {
                var preferencias = [];
                var candidatos_pre = _seleccion["preferencias"][categoria.codigo];
            ...
            if (typeof(categoria.max_tachas) !== "undefined") {
                var tachas = [];
                var candidatos_tac = _seleccion["tachas"];
